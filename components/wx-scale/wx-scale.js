Component({
  /**
   * 组件的属性列表
   */
  properties: {
    // 最小值
    min: {
      type: Number,
      value: 0
    },
    //最大值
    max: {
      type: Number,
      value: 100
    },
    // 是否开启整数模式
    "int": {
      type: Boolean,
      value: true
    },
    // 每个格子的长度（只能是 1 ，2 ，5 一个能被10整除的数字 ）
    step: {
      type: Number,
      value: 1
    },
    // 每个格子的实际行度 （单位px ，相对默认值）
    single: {
      type: Number,
      value: 8
    },
    // 卡尺左右的余量 ，最为60
    fiexNum: {
      type: Number,
      value: 0
    },
    // 高度
    h: {
      type: Number,
      value: 120
    },
    // 当前选中
    active: {
      type: null,
      value: '0',
      observer(newVal, oldVal, changedPath) {
        console.log(oldVal)
        let rul = this.data.rul
        rul.active = newVal
        console.log(newVal, 'new')
        let centerNum = this.assignValue(this, rul)
        this.setData({
          centerNum,
          rul
        })
      }
    },
    styles: {
      type: Object,
      value: {
        line: '#ffffff',
        bginner: '#ffffff',
        bgoutside: '#ffffff',
        lineSelect: '#52b8f5',
        font: '#404040'
      }
    }

  },

  /**
   * 组件的初始数据
   */
  data: {
    imageWidth: '',
    fiexNum: 0,
    bgoutside: '#18ff06',
    lineSelect: '#52b8f5',
    scaleId: '',
    rul: {},
    assingOldVal: -1,
    system:''
  },
  ready() {
    let that = this
    // 每次初始化 全局变量
    this._init()
    wx.getSystemInfo({
      success: function (res) {  // 获取手机信息
       let  a = res.system.split(" ")[0];
        console.log(a)
        that.setData({
          system:a
        })

      }
    })
  },

  /**
   * 组件的方法列表
   */
  methods: {
    /**
     * 绘制
     * 生成卡尺
     */
    draw(num, total, {self, rul}) {
      let that = this
      let canvasHeight = 80;
      let ctx = wx.createCanvasContext('canvas', self);
      //  绘制背景
      ctx.save()
      ctx.setFillStyle(rul.styles.bginner)
      ctx.fillRect(0, 0, total, canvasHeight)
      ctx.restore()
      ctx.beginPath()
      ctx.setLineWidth(1)
      ctx.setStrokeStyle(rul.styles.line)
      ctx.moveTo(rul.FIXED_NUM / 2, 0)
      ctx.lineTo(total - rul.FIXED_NUM / 2, 0)
      ctx.stroke()
      console.log('每一个的角度',rul.single)
      // console.log('每个客服的mun,',rul.FIXED_NUM)
      for (let i = 0; i < rul.unitNum + 1; i++) {  //循环1001次  //i=0

        if (that.data.system=='iOS'){
          // 绘制文字
          if (i % 10 === 0) {
            ctx.setFontSize(26)
            ctx.setFillStyle(rul.styles.font)
            ctx.setTextAlign('center')
            ctx.fillText(i/10, (i * rul.single * rul.step)+9, canvasHeight - 15)
          }
          // 绘制刻度
          if (i % 10 === 0) {
            ctx.beginPath()
            ctx.setLineWidth(2)
            ctx.setStrokeStyle(rul.styles.line)
            ctx.moveTo((i * rul.spa)+9, -2)
            ctx.lineTo( (i * rul.spa)+9, canvasHeight / 2)
            ctx.stroke()
          }else if(i % 5 === 0){
            ctx.beginPath()
            ctx.setLineWidth(2)
            ctx.setStrokeStyle(rul.styles.line)
            ctx.moveTo( (i * rul.spa)+9, -2)
            ctx.lineTo( (i * rul.spa)+9, canvasHeight -50)
            ctx.stroke()
          } else {
            ctx.beginPath()
            ctx.setLineWidth(1)
            ctx.setStrokeStyle(rul.styles.line)
            ctx.moveTo( (i * rul.spa)+9, -2)
            ctx.lineTo((i * rul.spa)+9, canvasHeight - 60)
            ctx.stroke()
          }
        }else{
          if (i % 10 === 0) {
            ctx.setFontSize(18)
            ctx.setFillStyle(rul.styles.font)
            ctx.setTextAlign('center')
            ctx.fillText(i/10, (i * rul.single * rul.step), canvasHeight - 15)
          }
          // 绘制刻度
          if (i % 10 === 0) {
            ctx.beginPath()
            ctx.setLineWidth(2)
            ctx.setStrokeStyle(rul.styles.line)
            ctx.moveTo((i * rul.spa), -2)
            ctx.lineTo( (i * rul.spa), canvasHeight / 2)
            ctx.stroke()
          }else if(i % 5 === 0){
            ctx.beginPath()
            ctx.setLineWidth(2)
            ctx.setStrokeStyle(rul.styles.line)
            ctx.moveTo( (i * rul.spa), -2)
            ctx.lineTo( (i * rul.spa), canvasHeight -50)
            ctx.stroke()
          } else {
            ctx.beginPath()
            ctx.setLineWidth(1)
            ctx.setStrokeStyle(rul.styles.line)
            ctx.moveTo( (i * rul.spa), -2)
            ctx.lineTo((i * rul.spa), canvasHeight - 60)
            ctx.stroke()
          }
        }


      }
      ctx.draw(true, setTimeout(() => {
        wx.canvasToTempFilePath({
          x: 0,
          y: 0,
          width: total,
          height: canvasHeight,
          canvasId: 'canvas',
          success: (res) => {
            // 改变高度重新计算
            rul.total = rul.total / 80 * rul.h
            rul.FIXED_NUM = rul.FIXED_NUM / 80 * rul.h
            let centerNum = this.assignValue(this, rul)
            self.setData({
              ruler: res.tempFilePath,
              centerNum,
              width: rul.total,
              h: rul.h,
              fiexNum: rul.FIXED_NUM,
              round: self.data.int ? rul.minNum : rul.minNum.toFixed(1),
              bgoutside: rul.styles.bgoutside,
              lineSelect: rul.styles.lineSelect,
            })
            console.log(this.data.ruler)
            self.triggerEvent('value', {
              value: self.data.round
            })
          },
          fail(e) {
            console.log(e)
          }
        }, this)
      }, 500));
    },
    /**
     * 获取滑动的值
     */
    bindscroll: function(e) {
      // console.log('111')
      let rul = this.data.rul
      // console.log(rul)
      // 移动距离
      let left = e.detail.scrollLeft;
      // 单格的实际距离
      let spa;
      // 判断是否是整数
      if (this.data.int) {
        spa = parseInt(rul.total - rul.FIXED_NUM) / rul.num * rul.step;
      } else {
        spa = parseFloat(rul.total - rul.FIXED_NUM) / rul.num / (rul.single / rul.step);
      }
      // 当前显示值
      let resultNum = Math.round(left / spa);
      // 还原为实际数值
      let redNum = Math.round(resultNum * spa)
      // 小数位处理
      resultNum = this.data.int ? resultNum * rul.step + rul.minNum : ((resultNum * rul.step) / 10 + rul.minNum).toFixed(1)
      if (this.data.assingOldVal === resultNum) return
      this.setData({
        round: resultNum,
        assingOldVal: resultNum
      })
      this.triggerEvent('value', {
        value: resultNum
      })
      clearTimeout(rul.Timer);
      rul.Timer = setTimeout(() => {
        // console.log("执行了定时器")
        this.setData({
          centerNum: redNum,
          round: resultNum,
          active: resultNum,
          assingOldVal: resultNum
        })
        this.triggerEvent('value', {
          value: resultNum
        })
      }, 200)
    },
    /**
     * 初始化卡尺
     */
    _init() {
      let self = this
      let rul = {
        spa: '', // 单个格子的距离
        unitNum: '', // 格子总数
        minNum: this.data.min, //最小值
        maxNum: this.data.max, //最大值
        num: this.data.max - this.data.min, // 仿数据总数
        FIXED_NUM: this.data.fiexNum, // 标尺左右空余部分
        single: this.data.single,  //单个格子的实际长度（单位px）一般不建议修改
        step: this.data.step,  //步长，相对传入的值，每个格子代表几个值（值只能是能被10整除的 整数 ，1，2，5, 10）
        h: this.data.h,   //自定义高度
        active: '',  //自定义选中位置 （三个值 min, max ,center , 范围内合法数值）  默认center
        styles: this.data.styles    //自定义卡尺颜色 注意： 仅支持 #dbdbdb 或者red 这种 颜色 不支持简写 如 #333
      }
      this._getErro(rul)
      //  获取节点信息，获取节点宽度
      var query = this.createSelectorQuery().in(this)
      query.select('#scale-wrapper').boundingClientRect((res) => {
        res.top // 这个组件内 #the-id 节点的上边界坐标
      }).exec((e) => {
        // console.log('初始化宽度')
        // 获节点宽度
        rul.windowWidth = e[0].height;
        // 判断是否使用整数类型
        // console.log(self.data.int) //判断用户选择的是否为整数类型
        if (self.data.int) {
          rul.unitNum = rul.num / rul.step;
        } else {
          // console.log(rul.num)  //100
          // console.log(rul.single) //10
          // console.log( rul.step)  //1
          // console.log(rul.single / rul.step)  //10
          rul.unitNum = rul.num * (rul.single / rul.step);
        }
        // console.log(rul.unitNum)  //1000  // 格子总数
        // 设置单个格子的长度
        //  通过控制rul.step ,这个值越大，那么就显示越稀疏，越小越密集
        rul.spa = rul.single * rul.step;
        // rul.spa = rul.single * 0.5;
        // console.log(rul.single)  ///单个格子的实际长度（单位px）一般不建议修改  //10
        // console.log(rul.step)  //1
        // console.log(rul.spa)  //10

        //每个格子的长度 * 格子的总数  + // 标尺左右空余部分
        rul.total = rul.spa * rul.unitNum + rul.FIXED_NUM
        // console.log(rul.total)   //按道理应该是尺子的长度
        self.setData({
          windowWidth: e[0].width,
          width: rul.total,
          fiexNum: rul.FIXED_NUM,
          rul
        })
        // console.log(rul.FIXED_NUM)  //60
        self.draw(rul.num, rul.total, {
          self,
          rul
        });
      })
    },
    /**
     * 输出错误信息
     */
    _getErro(rul) {
      // 判断 最大值 最小值 是否 正确
      if (rul.minNum >= rul.maxNum) {
        // console.error("您输入的最大值 小于最小值，请检查 minNum ， maxNum")
        rul.minNum = 0;
        rul.maxNum = 100
        rul.num = rul.maxNum - rul.minNum
      }
      // 判断 是否开启整数类型
      if (rul.step !== 1 && rul.step !== 2 && rul.step !== 5) {
        // console.error("步长只能是 1 ，2  ，5  ,请检查 step")
        rul.step = 1
      }
      if (rul.FIXED_NUM < 60) {
        // console.warn('左右余量 输入小于 60 ，可能影响显示效果，请检查 fiexNum')
        if (!rul.FIXED_NUM) {
          rul.FIXED_NUM = 60
        }
        if (rul.FIXED_NUM < 0) {
          // console.error('左右余量最小为0  ，请检查 fiexNum')
          rul.FIXED_NUM = 0;
        }
      }
      if (rul.single < 10) {
        // console.warn('格子单位小于10 ，可能影响显示效果，请检查 single')
        if (!rul.single) {
          rul.single = 10
        }
      }
      if (rul.h < 50) {
        console.warn('格子单位小于50 ，可能影响显示效果，请检查 h')
        if (!rul.h) {
          rul.h = 80
        }
        if (rul.h < 20) {
          console.error('高度最小为20  ，请检查 h')
          rul.h = 20;
        }
      }
      // 当前选中位置设置
      if (this.data.active === 'min') {
        rul.active = rul.minNum
      } else if (this.data.active === 'max') {
        rul.active = rul.maxNum
      } else if (this.data.active === 'center') {
        rul.active = (rul.maxNum + rul.minNum) / 2
      } else {
        rul.active = this.data.active
      }
      if (this.data.active !== 'min' && this.data.active !== 'max' && this.data.active !== 'center') {
        // console.log("任意数值")
        if (rul.active < rul.minNum || rul.active > rul.maxNum) {
          console.error('您输入的数值（active）超入范围，请检查 active')
        }
        if (rul.active % rul.step !== 0 && rul.int) {
          console.warn("您输入的数值（active）不是合法数值，请检查，所以导致结果可能有错误")
        }
        if (rul.active * 10 % rul.step !== 0 && !rul.int) {
          console.warn("您输入的数值（active）不是合法数值，请检查，所以导致结果可能有错误")
        }
      }

      if (!rul.styles) {
        rul.styles = {}
        if (!rul.styles.line) {
          rul.styles.line = '#ffffff'
        }
        if (!rul.styles.lineSelect) {
          rul.styles.lineSelect = '#52b8f5'
        }
        if (!rul.styles.bginner) {
          rul.styles.bginner = '#ffffff'
        }
        if (!rul.styles.bgoutside) {
          rul.styles.bgoutside = '#ffffff'
        }
        if (!rul.styles.font) {
          rul.styles.font = '#404040'
        }
      } else {
        if (!rul.styles.line) {
          rul.styles.line = '#ffffff'
        }
        if (!rul.styles.lineSelect) {
          rul.styles.lineSelect = '#52b8f5'
        }
        if (!rul.styles.bginner) {
          rul.styles.bginner = '#ffffff'
        }
        if (!rul.styles.bgoutside) {
          rul.styles.bgoutside = '#ffffff'
        }
        if (!rul.styles.font) {
          rul.styles.font = '#404040'
        }
      }
    },
    assignValue(self, rul) {
      return self.data.int ?
          ((rul.active - rul.minNum) / rul.step) *
          parseInt(rul.total - rul.FIXED_NUM) / rul.num * rul.step :
          ((rul.active - rul.minNum) * 10 / rul.step) *
          parseFloat((rul.total - rul.FIXED_NUM)) / rul.num / (rul.single / rul.step)
    }
  }
})